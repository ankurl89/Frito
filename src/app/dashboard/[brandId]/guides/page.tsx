...
/**
 * Founder Playbook — the guides library. Grouped by category, premium cards.
 */

import { useParams } from "next/navigation";
import Link from "next/link";
import { GUIDES, GUIDE_CATEGORIES, guidesByCategory, Guide } from "@/lib/guides";
import {
    ...
    <div className="min-h-full bg-[#F5F5F0]">
      ...
      <div className="bg-zinc-900 text-white px-8 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-4xl md:text-5xl font-black tracking-tight mb-3">Learn to sell.</h1>
          <p className="text-zinc-400 text-lg max-w-2xl">
            Real, no-fluff playbooks for first-time founders — from your first sale to ads, content, and repeat customers. Built for apparel brands in India.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-8 py-10 space-y-10">
          ...
          return (
            <section key={cat.key}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {guides.map(g => <GuideCard key={g.slug} guide={g} brandId={brandId} />)}
              </div>
            </section>
          );
        })}
        ...
        <p className="text-center text-xs text-zinc-400 pt-2">
          More guides coming. Got a topic you want covered? It helps us help you sell.
        </p>
      </div>
    </div>
  );
